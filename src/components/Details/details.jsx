...
export default function Detail() {
  ...
  const formErrorHandle = (event) => {
    if (event.target.id == commentLocal) {
      setComment(null);
    } else {
                ...
                <div className={style.minibox}>
                  ...
                  <ul className={style.genderBox}>
                    {videogame.Genregames?.map((obj) => {
                      return (
                        <li className="btn btn-danger" key={obj.name}>
                          {obj.name}
                        </li>
                      );
                    })}
      ...
      <div className={style.container}>
        ...
        <div className={style.minibox}>
          ...
          {videogame.ComentariosVs?.map((comment) => (
            <div key={comment.id}>
              <button onClick={formErrorHandle} id={comment.id}>
                {" "}
                {comment.id == commentLocal ? "x" : "!"}{" "}
              </button>
              {comment.id == commentLocal ? (
                <div>
                  ...
                  <button id={comment.id} value={"others"} onClick={sendReport}>
                    others
                  </button>
                </div>
              ) : null}
